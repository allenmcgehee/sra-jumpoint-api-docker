...
RUN apt-get install -y libatk-bridge2.0-dev
RUN apt-get install -y libcups2-dev
RUN apt-get install -y libxcomposite-dev
...
ARG BT_API_HOST
ARG BT_CLIENT_ID
ARG BT_CLIENT_SECRET
ARG BT_JP_CODE_NAME
ENV BT_API_HOST=${BT_API_HOST}
ENV BT_CLIENT_ID=${BT_CLIENT_ID}
ENV BT_CLIENT_SECRET=${BT_CLIENT_SECRET}
ENV BT_JP_CODE_NAME=${BT_JP_CODE_NAME}

COPY <<__eof__ /start.sh
#!/bin/bash
set -ex
cd tmp
curl -fsSLo btapi.zip https://\$BT_API_HOST/api/config/v1/cli/linux64-x86 && unzip btapi.zip
./btapi download jumpoint/\$(echo "code_name=\$BT_JP_CODE_NAME" | ./btapi -k list jumpoint | grep id | awk -F ':' '{print \$2}' | sed 's/[, ]//g')/installer
...
sh /tmp/bomgar-jpt-*.bin --install-dir \$HOME/jpt
cp /tmp/jumpzone.ini \$HOME/jpt/
\$HOME/jpt/bomgar-jpt --restart-loop 1000 --service
__eof__
